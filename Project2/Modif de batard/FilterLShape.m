...
for i =1:5
    ...
    elseif(study==3 )
        [xPhys, Mnd, loop, Compliance, Svm] = top88(4*nx0,4*ny0,volfrac,penal,rmin_study(i),ft);  
    end
    xPhys_plot{i} = xPhys;
    Mnd_plot{i} = Mnd;
    loop_plot{i} = loop;
...
Iter5 = 1:loop_plot{5};
figure(5);
CompPlot = myPlot(Iter5, Compliance_plot{5}, 'Iteration', 'Compliance');
matlab2tikz('ComplianceCase5.tex','width', '0.75\textwidth', 'height', '0.35\textwidth');
figure(55);
colormap(gray); imagesc(1-xPhys_plot{5}); caxis([0 1]); axis square;axis off;
%hold on; myPlot(0,0);
print('OptimisedGeomCase5','-depsc');
figure(555)
colormap(jet); imagesc(Svm_plot{5}); caxis([0 1]); axis square;cb2=colorbar;cb2.TickLabelInterpreter = 'latex'; axis off;
%hold on; myPlot(0,0);
set(gca,'fontsize', 20);
print('SigmaVMCase5','-depsc');
%